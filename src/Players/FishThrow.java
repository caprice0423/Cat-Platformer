...
public class FishThrow extends Attacking {
	private float movementSpeed;
	private Stopwatch existenceTimer = new Stopwatch();

	public FishThrow(Point location, float movementSpeed, int existenceTime) {
		super(location.x, location.y, new SpriteSheet(ImageLoader.load("deadFish.png"), 30, 30), "DEFAULT");
		...
		return new HashMap<String, Frame[]>() {
			{
				put("DEFAULT", new Frame[] {
						new FrameBuilder(spriteSheet.getSprite(0, 0), 0).withScale(1).withBounds(1, 1, 5, 5).build() });
			}
		};
	}
}
